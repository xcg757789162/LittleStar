...
describe('ClassroomCache', () => {
  ...
  describe('saveClassroom', () => {
    it('should save a classroom with knowledgeNodeId and date', async () => {
      ...
      const retrieved = await cache.getClassroom('kn-counting-1-5', 1, '2026-04-08')
    ...
    it('should overwrite existing classroom for same key', async () => {
      ...
      await cache.saveClassroom('kn-test', 1, '2026-04-08', classroom1)
      await cache.saveClassroom('kn-test', 1, '2026-04-08', classroom2)

      const retrieved = await cache.getClassroom('kn-test', 1, '2026-04-08')
  ...
  describe('getClassroom', () => {
    it('should return null for non-existent classroom', async () => {
      const result = await cache.getClassroom('nonexistent', 1, '2026-04-08')
    ...
    it('should retrieve classroom by knowledgeNodeId and date', async () => {
      ...
      await cache.saveClassroom('kn-animals', 1, '2026-04-09', classroom)

      const result = await cache.getClassroom('kn-animals', 1, '2026-04-09')
    ...
    it('should not cross-match different dates', async () => {
      const classroom = createMockClassroom()
      await cache.saveClassroom('kn-test', 1, '2026-04-08', classroom)

      const result = await cache.getClassroom('kn-test', 1, '2026-04-09')
      expect(result).toBeNull()
    })
  })
  ...
  describe('listCachedClassrooms', () => {
    ...
    it('should list all cached classrooms', async () => {
      await cache.saveClassroom('kn-1', 1, '2026-04-08', createMockClassroom({ id: 'c1' }))
      await cache.saveClassroom('kn-2', 1, '2026-04-08', createMockClassroom({ id: 'c2' }))
      await cache.saveClassroom('kn-3', 1, '2026-04-09', createMockClassroom({ id: 'c3' }))
    ...
    it('should filter by date when provided', async () => {
      await cache.saveClassroom('kn-1', 1, '2026-04-08', createMockClassroom({ id: 'c1' }))
      await cache.saveClassroom('kn-2', 1, '2026-04-09', createMockClassroom({ id: 'c2' }))
    ...
    it('should hide placeholder classrooms without renderable scenes from the lesson list', async () => {
      await cache.saveClassroom('kn-valid', 1, '2026-04-08', createMockClassroom({ id: 'valid-1', title: '数字王国' }))
      await cache.saveClassroom('kn-invalid', 1, '2026-04-08', createMockClassroom({
        id: 'invalid-1',
    ...
    it('should evict legacy classrooms that only contain scene.slides from the lesson list', async () => {
      await cache.saveClassroom('kn-legacy', 1, '2026-04-08', createLegacyClassroom({ id: 'legacy-1' }))

      await expect(cache.listCachedClassrooms()).resolves.toEqual([])
      await expect(cache.getClassroom('kn-legacy', 1, '2026-04-08')).resolves.toBeNull()
    })
  })

  describe('deleteClassroom', () => {
    it('should delete a specific cached classroom', async () => {
      await cache.saveClassroom('kn-test', 1, '2026-04-08', createMockClassroom())

      await cache.deleteClassroom('kn-test', 1, '2026-04-08')

      const result = await cache.getClassroom('kn-test', 1, '2026-04-08')
      expect(result).toBeNull()
    })

    it('should not throw when deleting non-existent classroom', async () => {
      await expect(
        cache.deleteClassroom('nonexistent', 1, '2026-04-08')
  ...
  describe('clearExpiredCache', () => {
    it('should remove classrooms older than specified date', async () => {
      await cache.saveClassroom('kn-old', 1, '2026-04-05', createMockClassroom({ id: 'old' }))
      await cache.saveClassroom('kn-recent', 1, '2026-04-08', createMockClassroom({ id: 'recent' }))

      await cache.clearExpiredCache('2026-04-07')

      const oldResult = await cache.getClassroom('kn-old', 1, '2026-04-05')
      const recentResult = await cache.getClassroom('kn-recent', 1, '2026-04-08')
    ...
    it('should not remove classrooms on or after the cutoff date', async () => {
      await cache.saveClassroom('kn-exact', 1, '2026-04-07', createMockClassroom({ id: 'exact' }))
      await cache.saveClassroom('kn-after', 1, '2026-04-08', createMockClassroom({ id: 'after' }))

      await cache.clearExpiredCache('2026-04-07')

      const exactResult = await cache.getClassroom('kn-exact', 1, '2026-04-07')
      const afterResult = await cache.getClassroom('kn-after', 1, '2026-04-08')
  ...
  describe('getCacheSize', () => {
    ...
    it('should support counting cache size by subject', async () => {
      await cache.saveClassroom('math-counting-1', 1, '2026-04-08', createMockClassroom({ id: 'math-1' }))
      await cache.saveClassroom('english-letter-a', 1, '2026-04-08', createMockClassroom({ id: 'english-1' }))
      await cache.saveClassroom('english-letter-b', 1, '2026-04-08', createMockClassroom({ id: 'english-2' }))
