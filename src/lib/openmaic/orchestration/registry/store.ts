...
import type { Participant, ParticipantRole } from '@/lib/openmaic/types/roundtable';
import { useUserProfileStore } from '@/lib/openmaic/store/user-profile';
import type { AgentInfo } from '@/lib/openmaic/generation/pipeline-types';
...
export async function loadGeneratedAgentsForStage(stageId: string): Promise<string[]> {
  const { getGeneratedAgentsByStageId } = await import('@/lib/openmaic/utils/database');
