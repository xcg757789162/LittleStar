...
import type { Stage, Scene, StageMode } from '@/types/openmaic/stage';
import { createSelectors } from '@/lib/openmaic/utils/create-selectors';
import type { ChatSession } from '@/types/openmaic/chat';
import type { SceneOutline } from '@/types/openmaic/generation';
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  setOutlines: (outlines) => {
    ...
    if (stageId) {
      import('@/lib/openmaic/utils/database').then(({ db }) => {
  ...
  saveToStorage: async () => {
    ...
    try {
      const { saveStageData } = await import('@/lib/openmaic/utils/stage-storage');
  ...
  loadFromStorage: async (stageId: string) => {
    try {
      ...
      const { loadStageData } = await import('@/lib/openmaic/utils/stage-storage');
      const data = await loadStageData(stageId);

      // Load outlines for resume-on-refresh
      const { db } = await import('@/lib/openmaic/utils/database');
