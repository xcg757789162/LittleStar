...
- placement-pregen-fixer：已确认 `build:server` 卡在 `src/services/config.ts(263)` 的 `import.meta.env` 访问，这段代码是 4 月 11 日已存在的 server-incompatible 写法；但本轮新增 `src/server/question-model.ts` 从 `config.ts` 引入 `BACKEND_LLM_PROVIDERS` 后，首次把它拉进了服务端编译链。已将后端 LLM provider 注册表抽到 `src/shared/backend-llm-providers.ts`，`config.ts` 改为 re-export，`question-model.ts` 改为依赖共享模块。最小验证：`npm run build:server` 通过，`vitest`（`question-model` + `ai-question-generator`）2 文件 / 6 测试通过，相关文件 `read_lints` 无新增错误。
- placement-pregen-fixer：现场 `littlestar-app` 容器里 `POST http://127.0.0.1:3003/api/pre-generate/question` 返回 `Cannot POST`，已确认不是 Nginx 代理问题，而是容器内 `/app/pregeneration/dist/server/index.js` 仍是旧构建。根因是 `docker/deploy/update-app.sh` 的 auto 模式会把 `src/server/*` 误判为仅前端更新，只跑 `vite build`，不会重建镜像内的 pregeneration 服务；另外 `docker/deploy/Dockerfile.app` 的 `pregen-builder` 在 Docker 内编译时还缺少 `src/lib/openmaic/media/types.ts`，导致 `src/services/openmaic/pipeline-types.ts` 依赖的 `MediaGenerationRequest` 无法解析。已补丁：1) `update-app.sh` auto 模式遇到 pregen 后端源码会改走 app rebuild，并新增 `/api/pre-generate/question` 路由探测；2) `Dockerfile.app` 最小补齐 `COPY src/lib/openmaic/media/types.ts`。实际验证：重跑 `bash docker/deploy/update-app.sh --full` 后，`docker compose ps -a` 中 `littlestar-app` 已 `Up (healthy)`，且宿主机 `POST http://localhost:8080/api/pre-generate/question` 与容器内 `POST http://127.0.0.1:3003/api/pre-generate/question` 均返回 **400**（已不再 404，说明新 question 路由生效）。
